...
class StateRegular(State):
    ...
    def __init__(self, identity, college, year, num_identity, num_college, num_year):
        ...
        self.year = year
        self.index = (identity * num_college * num_year) + (college * num_year) + year
...
class StateTerminal(State):
    ...
    def __init__(self, num_identity, num_college, num_year):
        ...
        State.__init__(self, num_identity, num_college, num_year)
        self.index = num_identity * num_college * num_year
...
class ActionDeliver(Action):
    ...
    def getIndex(self):
        return 3 + self.num_identity + self.num_college + self.num_year + \
            (self.identity * self.num_college * self.num_year) + \
            (self.college * self.num_year) + self.year
...
class PomdpGenerator(object):
    ...
    def computeRewardFunction(self, num_identity, num_college, num_year,
        ...
        for action in self.action_set:
            ...
            num_state = num_identity * num_college * num_year + 1
        ...
        for i in range(num_identity):
            for p in range(num_college):
                for r in range(num_year):
                    ret.append(StateRegular(i, p, r, num_identity, num_college, num_year))

        ret.append(StateTerminal(num_identity, num_college, num_year))
    ...
    def computeActionSet(self, num_identity, num_college, num_year):
        ...
        for p in range(num_college):
            ret.append(ActionAskPolarcollege(p, num_identity, num_college, num_year))

        for r in range(num_year):
            ret.append(ActionAskPolaryear(r, num_identity, num_college, num_year))

        for i in range(num_identity):
            for p in range(num_college):
                for r in range(num_year):
                    ret.append(ActionDeliver(i, p, r, num_identity, num_college,
                        num_year))
...
def main():
    ...
    # all
    weight_i = np.array([[1.00, 0.75, 0.50], 
                         [0.75, 1.00, 0.50], 
                         [0.50, 0.50, 1.00]])
    ...
    weight_y = np.array([[1.0, 0.6, 0.67], 
                         [0.14, 1.0, 0.4], 
                         [0.11, 0.27, 1.0]])
